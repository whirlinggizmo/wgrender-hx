...
`make` appears once, for the web: there wgrender is a wasm host the page loads, built
by emcc from the library wgrender's own Makefile produces. The native build has no
make and no generated build file -- project/Build.xml in the library compiles
wgrender with the same toolchain as the guest, and -D WGRENDER_DIR is the only thing
this has to tell it.
...
class Project:
    def __init__(self, root, name, entry, title=None, background='#f5f5f5', defines=()):
        self.root = pathlib.Path(root).resolve()
        self.name = name
        self.entry = entry  # the Haxe class with main(); `main` here is the CLI
        self.title = title or name
        self.background = background
        # Extra -D for both targets. One example needs one (fetch, for the TLS stack),
        # and a define that only reached one target would be the kind of asymmetry
        # that shows up much later as "works on the web".
    ...
    def generate(self):
        ...
        (self.root / 'guest.hxml').write_text(GUEST_HXML.format(name=self.name, entry=self.entry, defines=self.define_lines()))
        (self.root / 'desktop.hxml').write_text(DESKTOP_HXML.format(name=self.name, entry=self.entry, defines=self.define_lines()))
    ...
    def build_desktop(self):
        """The same guest, native: hxcpp compiles wgrender and the glue into one binary.

        There is no `make` here and no generated build file. project/Build.xml in the
        library already knows how to build wgrender; the only thing this has to say is
        which one, and -D WGRENDER_DIR says it. That is the same path an installed
        copy takes, so a checkout is not exercising a build nobody else runs.
        """
        self.generate()
        self.check_binding()
        ...
        (build / 'desktop.hxml').write_text(
            '# Generated by guestbuild.py. project/Build.xml in wgrender-hx compiles\n'
            '# wgrender with the same toolchain as the guest; this names which one.\n'
            f'-D WGRENDER_DIR={self.wgrender}\n')
