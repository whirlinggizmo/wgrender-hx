...
const run = new RunProcesses("particles");
const errors = [], lines = [];
try {
    ...
    // click: the 2D confetti emitter bursts where the pointer is
    for (const type of ["mousePressed", "mouseReleased"])
        await page.send("Input.dispatchMouseEvent", { type, x: mx, y: my, button: "left", clickCount: 1 });
    await sleep(400);
