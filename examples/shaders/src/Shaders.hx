...
// one can reach:
//
//   toon        lights in flat bands with a rim light, on the *animated* character --
...
class Shaders {
	...
	/** Slot 1 is the character's body; slot 0 is its blob shadow, left alone. **/
	...
	static function addModels():Void {
		...
		character = Model.create(Handle.NONE); // the mesh attaches when it loads
		Model.setTransform(character, new Vec3(-1.9, FLOOR_Y, 0), new Vec3(0, 0.4, 0),
	...
	static function onShader(which:Int, path:String):Void {
		...
		switch which {
			case ASSET_TOON:
				...
				Model.setMaterial(character, CHARACTER_BODY_SLOT, material);
