...
"""Build the wgrender simple example in Haxe (Haxe -> hxcpp -> C++ -> native/wasm).
    ...
    ./build.py all         both
    ./build.py check       run test/CheckBindings.hx against headless wgrender
...
def check():
    ...
    Headless is wgrender's own test target (`make HEADLESS=1`): no window, GPU or
...
<!-- Generated by build.py. Headless links no window, GPU or audio. -->
  <files id="haxe">
...
""")
    (build / 'check.hxml').write_text(
        '# Generated by build.py — the headless library, for the checks.\n'
        f'-D WGR_BUILD_XML={build / "check.xml"}\n')
    run([HAXE, 'check.hxml'])
    binary = ROOT / 'build/cpp/check/check-bindings'
