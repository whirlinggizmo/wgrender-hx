...
"""Build the wgrender simple example in Haxe (Haxe -> hxcpp -> C++ -> native/wasm).
    ...
    ./build.py all         both
    ./build.py check       compile the whole binding surface (test/CheckBindings.hx)
...
def check():
    """Everything in wgr.Wgr compiles, not just what Simple.hx happens to use."""
    if not (ROOT / 'build/desktop.hxml').exists():
        build_desktop()
    run([HAXE, 'check.hxml'])
    print('binding surface compiles')
